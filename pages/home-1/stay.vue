...
// Filter for accommodation businesses
const accommodationBusinesses = computed(() => {
...
<template>
    <section class="container mb-sm-5 mb-4 pb-lg-4">
        <div class="d-sm-flex align-items-center justify-content-between mb-4 pb-2">
            <h2 class="h3 mb-sm-0">Onde ficar</h2>
        ...
        <div v-else-if="accommodationBusinesses.length === 0" class="text-center py-4">
            <p class="text-muted">Nenhum alojamento disponível no momento.</p>
        </div>
        
        <swiper v-else :modules="modules" :breakpoints="swiperOptions.breakpoints" navigation :loop="true" class="h-100vh">
            <SwiperSlide v-for="business in accommodationBusinesses" :key="business.id">
